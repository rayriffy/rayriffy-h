    ...
    <div className='py-0 md:py-2'>
      ...
      <div className='flex flex-wrap'>
        ...
        <div className='w-full md:w-7/12 pt-0 md:pt-4 px-4'>
          <div className='bg-white dark:bg-gray-800 p-6 rounded-lg'>
            <div className='text-3xl font-semibold text-gray-900 dark:text-white leading-tight'>{hentai.title.pretty}</div>
            <div className='text-sm font-semibold text-gray-600 dark:text-gray-400 pt-2'>{hentai.title.japanese}</div>
            <div className='pt-4'>
              {tagStack.map(stack => {
      ...
      <div className='pt-6 flex justify-center'>
        <div className='w-full md:w-10/12 lg:w-8/12 xl:w-6/12'>
          {hentai.images.pages.map((page, i) => !raw.exclude.includes(i + 1) ? (
            <div className='overflow-hidden' key={`reader-${raw.raw.id}-page-${i + 1}`}>
              <BluredImage
                height={page.h}
                width={page.w}
                alt={`Page ${i + 1}`}
