    ...
    <React.Fragment>
      ...
      <div className='pt-4 pb-2 flex justify-end flex-wrap'>
        <button
          ...
          {exportStat === null || exportStat === 'load' || exportStat === 'fail' ? 'Cancel' : 'Close'}
        </button>
        {exportStat === null || exportStat === 'load' || exportStat === 'fail' ? (
          <button
            className={`${exportStat !== 'load' ? 'bg-blue-500 hover:bg-blue-700 cursor-pointer text-white' : 'bg-gray-500 hover:bg-gray-700 cursor-not-allowed text-white dark:text-gray-800'} font-bold py-2 px-4 rounded transition-all duration-200 select-none`}
