...
const Dropdown: React.FC<{
  ...
  return (
    <div className="relative">
      ...
      <div
        className={`absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg transition-all duration-200 ${
          open ? `opacity-100 z-${zIndex}` : 'opacity-0 z-hide'
        }`}
      >
        <div
...
export const Navigation: React.FC<Props> = props => {
  ...
  return (
    <div className="relative">
      <nav
        className={`absolute top-0 left-0 right-0 flex-grow md:block px-4 pt-2 pb-3 bg-white dark:bg-gray-800 md:dark:bg-transparent transition-all duration-200 md:opacity-100 ${
        ...
        >
          {tagStacks.map(tag => (
            ...
            >
              <div
                className={`${
                  subtitle.startsWith(`${tag.name}/`)
                    ? 'bg-gray-200 dark:bg-gray-700'
                    : 'bg-transparent dark:bg-transparent'
                } capitalize block px-4 py-2 mt-2 text-sm font-semibold rounded-lg dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline`}
              >
                {tag.name}
              </div>
            </TransparentLink>
